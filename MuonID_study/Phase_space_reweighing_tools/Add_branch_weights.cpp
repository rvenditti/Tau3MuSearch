...
void Add_branch_weights(){
    ...
    for(auto i=0; i<n_bkg; i++){
        upd(inputpath_bkg[i]);
        std::cout<<"Updated input file: "<<inputpath_bkg[i]<<std::endl;
    }
    //MC DsTau3Mu and B0Tau3Mu
    upd(inputpath_Ds);
    std::cout<<"Updated input file: "<<inputpath_Ds<<std::endl;
    upd(inputpath_B0);
    std::cout<<"Updated input file: "<<inputpath_B0<<std::endl;
}
