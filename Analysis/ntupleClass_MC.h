...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   TBranch        *b_FlightDistPVSV_chi2;   //!
    ...
    virtual Double_t MuonFinderGen1(Double_t pt, Double_t eta, Double_t phi);
    virtual Double_t MuonFinderGen2(Double_t pt, Double_t eta, Double_t phi);
    virtual Double_t MuonFinderGen3(Double_t pt, Double_t eta, Double_t phi);
    ...
    virtual void     DiMuMass(TH1D *h_Zero, TH1D *h_Zero2, Double_t ptmin);
    virtual Double_t TriMuonMass(Double_t pt1, Double_t pt2, Double_t pt3, Double_t eta1, Double_t eta2, Double_t eta3, Double_t phi1, Double_t phi2, Double_t phi3, Double_t mass);
    ...
    virtual void    QuadMuMass(TH1F *h, TH1F *h_Zero);
    ...
    virtual Bool_t  isPairNotAOmega(Double_t dimu1_2, Double_t dimu2_3, Double_t dimu1_3, Double_t sigma);
    ...
    virtual void    cutName(TString listCut[8]);
    virtual void    cutName_New(TString listCut[8]);
    virtual void    PdgIdHisto(TCanvas *canv, TH1I *hist, Int_t Idsummary[38], TString pIdList[38]);
    virtual void    CutEffHisto(TCanvas *canv, TH1I *hist, Int_t cut[8], TString listCut[8]);
...
//When run inside an event, given the characteristics of a muon GEN1 (pt, eta, phi), the function return the # of the corresponding GENmuon in the event
Double_t ntupleClass_MC::MuonFinderGen1(Double_t pt, Double_t eta, Double_t phi){
    int n=0, m=-999;
    ...
    if(n>1) cout << "Error: There is more than 1 muonGEN(1) that matches the conditions!" << endl;
    if(n==0) cout << "Error: There are NO muonGEN(1) that match the conditions!" << endl;
    return m;
}

//When run inside an event, given the characteristics of a muon GEN2 (pt, eta, phi), the function return the # of the corresponding GENmuon in the event
Double_t ntupleClass_MC::MuonFinderGen2(Double_t pt, Double_t eta, Double_t phi){
    int n=0, m=-999;
    ...
    if(n>1) cout << "Error: There is more than 1 muonGEN(2) that matches the conditions!" << endl;
    if(n==0) cout << "Error: There are NO muonGEN(2) that match the conditions!" << endl;
    return m;
}

//When run inside an event, given the characteristics of a muon GEN3 (pt, eta, phi), the function return the # of the corresponding GENmuon in the event
Double_t ntupleClass_MC::MuonFinderGen3(Double_t pt, Double_t eta, Double_t phi){
    int n=0, m=-999;
    ...
    if(n>1) cout << "Error: There is more than 1 muonGEN(3) that matches the conditions!" << endl;
    if(n==0) cout << "Error: There are NO muonGEN(3) that match the conditions!" << endl;
    return m;
}
...
//Given the energy, eta, phi, pt of 3 muons it returns their invariant mass
Double_t ntupleClass_MC::TriMuonMass(Double_t pt1, Double_t pt2, Double_t pt3, Double_t eta1, Double_t eta2, Double_t eta3, Double_t phi1, Double_t phi2, Double_t phi3, Double_t mass){
    TLorentzVector mu1, mu2, mu3, mutot;
    mu1.SetPtEtaPhiM(pt1, eta1, phi1, mass);
    mu2.SetPtEtaPhiM(pt2, eta2, phi2, mass);
    ...
    for(int k=0; k<MuonPt->size(); k++){
        if(k != mu1 && k!= mu2 && k!= mu3){
            if(Muon_isTrackerMuon->at(k) == 1 && MuonPt->at(k) > 0.5){
                if (isPairDeltaZGood(Muon_vz->at(k), Muon_vz->at(mu1), Muon_vz->at(mu2), 0.5) == true && isDeltaZGood(Muon_vz->at(k), Muon_vz->at(mu3), 0.5) == true){
...
// Given the dimuon mass and a value for sigma, the function return true if the dimuon mass is NOT compatible w/ the mass of an Omega(782) in 2 sigmas
Bool_t ntupleClass_MC::isNotAOmega(Double_t dimumass, Double_t sigma){
    const double OmegaMass = 0.78265; //Omega Mass in GeV
    if (dimumass < (OmegaMass-2*sigma) || dimumass > (OmegaMass+2*sigma))
        return true;
    else return false;
}

//Given 3 muons it checks, for all the pairs having opposite charges, if the corresponding dimuon mass is NOT compatible w/ the Omega(782)mass
Bool_t ntupleClass_MC::isPairNotAOmega(Double_t dimu1_2, Double_t dimu2_3, Double_t dimu1_3, Double_t sigma){
    if(dimu1_2!=0 && dimu2_3!=0 && dimu1_3!=0){
        cout << "ERROR!!! There are 3 pairs of muons w/ opposite charges !!!" << endl;
        return false;
    }
    else if (dimu1_2!=0 && dimu2_3!=0 && dimu1_3==0){
        if (isNotAOmega(dimu1_2,sigma) == true && isNotAOmega(dimu2_3,sigma) == true)
            return true;
        else return false;
    }
    else if(dimu1_2!=0 && dimu2_3==0 && dimu1_3!=0){
        if (isNotAOmega(dimu1_2,sigma) == true && isNotAOmega(dimu1_3,sigma) == true)
            return true;
        else return false;
    }
    else if(dimu1_2==0 && dimu2_3!=0 && dimu1_3!=0){
        if (isNotAOmega(dimu2_3,sigma) == true && isNotAOmega(dimu1_3,sigma) == true)
            return true;
        else return false;
    }
    else if(dimu1_2!=0 && dimu2_3==0 && dimu1_3==0){
        if (isNotAOmega(dimu1_2,sigma) == true)
            return true;
        else return false;
    }
    else if(dimu1_2==0 && dimu2_3!=0 && dimu1_3==0){
        if (isNotAOmega(dimu2_3,sigma) == true)
            return true;
        else return false;
    }
    else if(dimu1_2==0 && dimu2_3==0 && dimu1_3!=0){
        if (isNotAOmega(dimu1_3,sigma) == true)
            return true;
        else return false;
    }
    cout << "C'è qualquadra che non cosa!" << endl;
...
void ntupleClass_MC::cutName(TString listCut[8]){
    ...
    listCut[7] = "DimuonMass-Omega";
}

// Given a vector of strings the function initializes it with the names of the cuts
void ntupleClass_MC::cutName_New(TString listCut[8]){
    //    List of the names of the cuts
    listCut[0] = "BeforeCuts";
    listCut[1] = "Chi2Triplet";
    listCut[2] = "2glb+1trk";
...
void ntupleClass_MC::CutEffHisto(TCanvas *canv, TH1I *hist, Int_t cut[8], TString listCut[8]){
    for(int k=0; k<8; k++){
        hist->Fill(k+1, cut[k]);
