...
void ntupleClass_Control::LoopControl(){
    ...
    for (Long64_t jentry=0; jentry<nentries; jentry++) {
        ...
        Ncut++; cutevt[Ncut]++; cut[Ncut] += selectedTripletsIndex->size();
        //CUT 2 : HLT fired
        ...
        for (int j=0; j<selectedTripletsIndex->size(); j++){
            ...
            if(RefittedPV2_NTracks->at(j) > 1){
                ...
                //CUT 4 : check that both mu are different from the track && Global
                if((DuplicateFinder(Mu01_Eta->at(mu_Ind[0]), Mu02_Eta->at(mu_Ind[1]), Tr_Eta->at(mu_Ind[2]), Mu01_Phi->at(mu_Ind[0]), Mu02_Phi->at(mu_Ind[1]), Tr_Phi->at(mu_Ind[2]), Mu01_Pt->at(mu_Ind[0]), Mu02_Pt->at(mu_Ind[1]), Tr_Pt->at(mu_Ind[2])) == true) && (Muon_isGlobal->at(mu[0]) == 1) && (Muon_isGlobal->at(mu[1]) == 1)){
                    ...
                    FillHistoStepByStep("MC", j, mu_Ind, mu, Ncut, hPt, hPt_mu, hEta, hEta_mu, hPhi, hVx, hVy, hVz, hPt_Tr, hEta_Tr, hPt_tripl, hEta_tripl, hPhi_tripl, hMass_tripl, IdsummaryDaughter, IdsummaryMother, Idsummary2D);
                    //CUT 5 : check condition on Chi2 vertex ( 0 < Chi2 < 15)
                    if (TripletVtx2_Chi2->at(j) < TripletVtx_Chi2max && TripletVtx2_Chi2->at(j) > 0 ){
                        Ncut++; cut[Ncut]++; cuttripl[Ncut]++;
                        FillHistoStepByStep("MC", j, mu_Ind, mu, Ncut, hPt, hPt_mu, hEta, hEta_mu, hPhi, hVx, hVy, hVz, hPt_Tr, hEta_Tr, hPt_tripl, hEta_tripl, hPhi_tripl, hMass_tripl, IdsummaryDaughter, IdsummaryMother, Idsummary2D);
                        //CUT 6 : check condition 2 mu w/ opposite charge
                        ...
                        if (dimu1_2 != 0){
                            Ncut++; cut[Ncut]++; cuttripl[Ncut]++;
                            FillHistoStepByStep("MC", j, mu_Ind, mu, Ncut, hPt, hPt_mu, hEta, hEta_mu, hPhi, hVx, hVy, hVz, hPt_Tr, hEta_Tr, hPt_tripl, hEta_tripl, hPhi_tripl, hMass_tripl, IdsummaryDaughter, IdsummaryMother, Idsummary2D);
                            //CUT 7 : check condition on invariant dimuon mass (1-1.04) GeV
                            if (dimu1_2 >= 1 && dimu1_2 <= 1.04){
                                Ncut++; cut[Ncut]++; cuttripl[Ncut]++;
                                FillHistoStepByStep("MC", j, mu_Ind, mu, Ncut, hPt, hPt_mu, hEta, hEta_mu, hPhi, hVx, hVy, hVz, hPt_Tr, hEta_Tr, hPt_tripl, hEta_tripl, hPhi_tripl, hMass_tripl, IdsummaryDaughter, IdsummaryMother, Idsummary2D);
                                //CUT 8 : check condition on IP of the track
                                if(Track_dz->at(mu[2]) < 20){
                                    Ncut++; cut[Ncut]++; cuttripl[Ncut]++;
                                    FillHistoStepByStep("MC", j, mu_Ind, mu, Ncut, hPt, hPt_mu, hEta, hEta_mu, hPhi, hVx, hVy, hVz, hPt_Tr, hEta_Tr, hPt_tripl, hEta_tripl, hPhi_tripl, hMass_tripl, IdsummaryDaughter, IdsummaryMother, Idsummary2D);
                                    //CUT 9 : check condition on IP of the track
                                    if (Track_dxy->at(mu[2]) < 0.3){
                                        ...
                                        if(1 == 1){
                                            ...
                                            if(Mu01_dRtriggerMatch->at(j)<0.03){
                                                Ncut++; cut[Ncut]++; cuttripl[Ncut]++;
                                                FillHistoStepByStep("MC", j, mu_Ind, mu, Ncut, hPt, hPt_mu, hEta, hEta_mu, hPhi, hVx, hVy, hVz, hPt_Tr, hEta_Tr, hPt_tripl, hEta_tripl, hPhi_tripl, hMass_tripl, IdsummaryDaughter, IdsummaryMother, Idsummary2D);
                                                // CUT 12 : Mu2 Trigger matching
                                                if(Mu02_dRtriggerMatch->at(j)<0.03){
                                                    Ncut++; cut[Ncut]++; cuttripl[Ncut]++;
                                                    FillHistoStepByStep("MC", j, mu_Ind, mu, Ncut, hPt, hPt_mu, hEta, hEta_mu, hPhi, hVx, hVy, hVz, hPt_Tr, hEta_Tr, hPt_tripl, hEta_tripl, hPhi_tripl, hMass_tripl, IdsummaryDaughter, IdsummaryMother, Idsummary2D);
        ...
        // N. events that passed each triplet selection
        ...
        if(ntripl > 0) {
            ...
            MatchIndex("ID", ind, mu_Ind, mu);
                ...
                cout<<"scaleFactor "<<scaleFactor.value<<" +- "<<scaleFactor.error<<endl;
...
void ntupleClass_Control::LoopControl_Data(TString type, TString datasetName){
    ...
    cout << "TOTAL N. EVENTS -> " << cutevt[0] << endl;
    cout << "TOTAL N. TRIPLETS -> " << cut[0] << endl;
    cout << "Triplets survived: " << cutevt[NCUTS-1] << endl;
    //Histo of cuts Efficiency
    TCanvas *canvEvt = new TCanvas("CutEfficiency_Nevents", "CutEfficiency_Nevents", 0, 0, 1200, 1000);
    Draw_CutEffCanvas(canvEvt, hCutEffEvt, cutevt, listCut);
    TCanvas *canv = new TCanvas("CutEfficiency_Ntriplets", "CutEfficiency_Ntriplets", 0, 0, 1200, 1000);
