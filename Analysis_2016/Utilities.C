...
Int_t ntupleClass_tau3mu::BestTripletFinder(std::vector< Int_t > triplIndex){
    ...
    int index = 0; double bestChi2 = 10000000.0;
...
void ntupleClass_tau3mu::Fill_CutName(TString listCut[NCUTS]){
    ...
    listCut[4] = "all mu are PF";
    listCut[5] = "3global_Pt>2_eta<2.4";
    listCut[6] = "DeltaR";
    listCut[7] = "DeltaZ";
    listCut[8] = "PV_SV displacement";
    listCut[9] = "MassTriplet";
    listCut[10] = "Veto #Phi mass";
    listCut[11] = "Veto #omega mass";
    listCut[12] = "Muon_innerTrack_ValidFraction";
    listCut[13] = "TriggerMatching";
    listCut[14] = "Muon_GlbTrk_Chi2<30";
    listCut[15] = "Min_segmComp>0.3";
    listCut[16] = "SVtx Chi2<300";

    listCut[17] = "Sideband_HLT_L1OR";
    listCut[18] = "Peak_HLT_L1OR";
    listCut[19] = "Full_HLT_L1OR";
}
...
void ntupleClass_tau3mu::FillHistoAC(Int_t ind, Int_t mu[NMU], TH1F *hSegmComp, TH1F *hfv_d3Dsig, TH1F *hChi2Track, TH1D *hNMatchedStat, TH1D *hFlightDist, TH1D *hFlightDist_Signif, TH2D *hFlightDistvsP, TH1D *hPtErrOverPt, TH1D *hmassdi, std::vector< Double_t > dimu, TH1F *hmassQuad, TH1F *hmassQuad_Zero){
...
void ntupleClass_tau3mu::FillHistoResoPt_AC(Int_t muGen[NMU], TH1D *hPtRes, TH1D *hPtRes_mu[NMU], TH1D *hPtResBarrel, TH1D *hPtResBarrel_mu[NMU], TH1D *hPtResEndcap, TH1D *hPtResEndcap_mu[NMU]){
    // Pt Reso After cuts
...
void ntupleClass_tau3mu::FillHistoStepByStep(bool isMC, Int_t ind, Int_t mu_Ind[NMU], Int_t mu[NMU], Int_t Ncut, Int_t l1double_fired, Int_t l1double_DoubleMu4_fired, Int_t l1double_DoubleMu0_fired, Int_t l1triple_fired, TH1D *hL1[NCUTS], TH1D *hPt[NCUTS], TH1D *hPt_mu[NCUTS][NMU], TH1D *hEta[NCUTS], TH1D *hEta_mu[NCUTS][NMU], TH1D *hPhi[NCUTS], TH1D *hVx[NCUTS], TH1D *hVy[NCUTS], TH1D *hVz[NCUTS], TH1D *hMass_pair[NCUTS], TH1D *hDeltaR_pair[NCUTS], TH1D *hDeltaZ_pair[NCUTS], TH1D *hPt_tripl[NCUTS], TH1D *hEta_tripl[NCUTS], TH1D *hPhi_tripl[NCUTS], TH1D *hMass_tripl[NCUTS], TH1D *hChi2_tripl[NCUTS], Int_t IdsummaryDaughter[NCUTS][NPARTICLES], Int_t IdsummaryMother[NCUTS][NPARTICLES], Int_t Idsummary2D[NCUTS][NPARTICLES][NPARTICLES]){
    ...
    if (l1double_DoubleMu4_fired == 1) hL1[Ncut]->Fill(4);
    if (l1double_DoubleMu0_fired == 1) hL1[Ncut]->Fill(5);
}
...
void ntupleClass_tau3mu::InitHistoBC(TH1D *&hMass_tripl, TH1D *&hChi2Vertex, TH2D *&hMassvsChi2, TH1F *&hMass_quad, TH1F *&hMass_quad_Zero, TH1D *&hMass_di, TH1D *&hMass_di2, TH1D *&hPtRes, TH1D *hPtRes_mu[NMU], TH1D *&hPtResBarrel, TH1D *hPtResBarrel_mu[NMU], TH1D *&hPtResEndcap, TH1D *hPtResEndcap_mu[NMU], TH1I *&hPdgId_Gen, TH1I *&hMotherPdgId_Gen, TH2I *&hPdgId2D_Gen){
    ...
    hMass_tripl->Sumw2();
    hChi2Vertex = new TH1D("Chi2Vertex", "Chi2Vertex", 20003, -1.5, 10000); // binning 0.5
...
void ntupleClass_tau3mu::InitHistoStepByStep_Triplet(TH1D *hL1[NCUTS], TH1D *hPt_tripl[NCUTS], TH1D *hEta_tripl[NCUTS], TH1D *hPhi_tripl[NCUTS], TH1D *hMass_tripl[NCUTS], TH1D *hChi2_tripl[NCUTS]){
    for(int i=0; i<NCUTS; i++){
        ...
        TString hL1Name = "L1 fired (Double=1, Triple=2, OR=3, DoubleMu4=4, DoubleMu0=5) cut"; hL1Name += i;
...
Bool_t ntupleClass_tau3mu::isPhi(std::vector<Double_t> dimu){
    ...
    for(int i = 0; i<dimu.size(); i++){
        ...
        if (dimu.at(i)<(1.04) && dimu.at(i)>(1.00)){ //Jian
            n++;
        }
    }
    if(n==0) return false;
...
Bool_t ntupleClass_tau3mu::isOmega(std::vector<Double_t> dimu){
    ...
    for(int i = 0; i<dimu.size(); i++){
        ...
        if (dimu.at(i)<(0.797) && dimu.at(i)>(0.767)){ //Jian
            n++;
        }
    }
    if(n==0) return false;
...
bool ntupleClass_tau3mu::trigMatchJian(Int_t tripIndex, Int_t mu[NMU], std::vector< std::array<double, 3> > Muon_HLT, bool isVerbose){
    // For each triplet, it loops on the muons and looks for a trigger object having dR<0.03 and dP/P<0.1
    // if all 3 muons have such a trigger object, the triplet is matched (return true)
    bool triplet_match = false;
    Float_t deltaR = 999;
    Float_t deltaP = 999;
    std::size_t NtrgObj = Muon_HLT.size();
    bool match[NtrgObj] = { false };
    int Nmatches = 0;

    //Loop on the trigger objects
    for( std::size_t k=0; k<Muon_HLT.size(); k++ ){
        //Loop on muons in the triplet
        for( int i = 0; i<NMU; i++ ){
             deltaR = dR( MuonEta->at(mu[i]), Muon_HLT[k][1], MuonPhi->at(mu[i]), Muon_HLT[k][2]);
             deltaP = std::abs(MuonPt->at(mu[i]) - Muon_HLT[k][0])/MuonPt->at(mu[i]);
             if(deltaR<0.03 && deltaP<0.1) {
                 match[k] = true;
             }
             if(isVerbose) cout<<" mu "<<i<<" TrigObj "<<k<<" dR "<<deltaR<<" dP/P "<<deltaP<<endl;
        }
        if(isVerbose) cout<<" TrigObj "<<k<<"match[k] "<<match[k]<<endl;
    }
    for( std::size_t j=0; j<NtrgObj; j++ ){
        if(match[j] == true) Nmatches++;
    }
    if(Nmatches>2) triplet_match = true;
    if(isVerbose) cout<<"number of matches "<<Nmatches<<" --> triplet_match="<<triplet_match<<endl;
    return triplet_match;
}
