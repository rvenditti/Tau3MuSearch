...
void ntupleClass_Control::LoopControl(TString type, TString datasetName){
    ...
    double w[6] = {0}; // weight for HLT in MC
    TString runMC = "part"; // Choose btw 'part', 'all', 'A', 'B', 'C', 'D'
    if(runMC.Contains("part")){
        w[0] = 0.124;
        w[1] = 0.156;
        w[2] = 0.21;
        w[3] = 0.449;
    ...
    else if(runMC.Contains("A")){
        w[0] = 0.095;
        w[1] = 0.095; // no HLT-Mu8IP3
        w[2] = 0.095; // no HLT-Mu8IP5
        w[3] = 0.095; // no HLT-Mu9IP5
        w[4] = 1.;
        w[5] = 1.; // no HLT-Mu12IP6
    }
    else if(runMC.Contains("B")){
        w[0] = 0.074;
        w[1] = 0.145;
        w[2] = 0.145; // no HLT-Mu8IP5
        w[3] = 0.348;
        w[4] = 0.876;
        w[5] = 1.;
    }
    else if(runMC.Contains("C")){
        w[0] = 0.029;
        w[1] = 0.029; // no HLT-Mu8IP3
        w[2] = 0.029; // no HLT-Mu8IP5
        w[3] = 0.489;
        w[4] = 0.696;
        w[5] = 1.;
    }
    else if(runMC.Contains("D")){
        w[0] = 0.242;
        w[1] = 0.242; // no HLT-Mu8IP3
        w[2] = 0.368;
        w[3] = 0.635;
        w[4] = 0.783;
        w[5] = 1.;
    }
    else if(runMC.Contains("all")){
        w[0] = 0.167;
        w[1] = 0.186;
        w[2] = 0.266;
        w[3] = 0.521;
...
//        cout << "Evt n. " << jentry+1 << endl;
        ...
        int hlt_old = 0, hlt_parking = 0, hlt[6] = {0}, hlt_Mu_fired[6] = {0}, goodTriplet = 0;
        ...
        for(int h=0; h<Trigger_hltname->size(); h++) {
            ...
            if( (hltName.Contains("HLT_Mu7_IP4") || hltName.Contains("HLT_Mu8_IP") || hltName.Contains("HLT_Mu9_IP") || hltName.Contains("HLT_Mu12_IP")) && Trigger_hltdecision->at(h) == 1){
                hlt_parking = 1;
                if(hltName.Contains("HLT_Mu7_IP4")) hlt[0] = 1;
                if(hltName.Contains("HLT_Mu8_IP3")) hlt[1] = 1;
                if(hltName.Contains("HLT_Mu8_IP5")) hlt[2] = 1;
                if(hltName.Contains("HLT_Mu9_IP5")) hlt[3] = 1;
                if(hltName.Contains("HLT_Mu9_IP6")) hlt[4] = 1;
                if(hltName.Contains("HLT_Mu12_IP6")) hlt[5] = 1;
            }
        } // end loop on HLT
        
        // Selecting HLT exclusively and reweighting MC
        if(hlt_parking == 1){
            for(int h=0; h<6; h++){
                if(isMC == true){
                    if(h==0 && rand <= w[0] && hlt[h] == 1) { hlt_Mu_fired[h] = 1; hHLT_deepStudy[1]->Fill(h); break; }
                    if(h!=0 && rand > w[h-1] && rand <= w[h] && hlt[h] == 1) { hlt_Mu_fired[h] = 1; hHLT_deepStudy[1]->Fill(h); break; }
                }
                else{ // data
                    if(hlt[h] == 1){ hlt_Mu_fired[h] = 1; hHLT_deepStudy[1]->Fill(h); break; }
                }
            }
        }
